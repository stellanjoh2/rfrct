...
echo "  2. Replace package.json in that history so @rfrct/core is not '*':"
...
echo "       (cd ../blod-wt && git add package.json && git commit -m 'fix: resolve @rfrct/core for standalone')"
echo "       git worktree remove ../blod-wt"
echo "  3. Push rfrct-core package to its own repo first — see apps/blod/STANDALONE.md"
